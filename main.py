...
class VaR:
    ...
    def data(self):
        ...
        # Handle single vs multiple assets - THIS IS THE FIX
        if len(self.ticker) == 1:
            # For single asset, structure is different
            if 'Adj Close' in df.columns:
                self.adj_close_df = df[['Adj Close']].rename(columns={'Adj Close': self.ticker[0]})
            else:
                self.adj_close_df = df[['Close']].rename(columns={'Close': self.ticker[0]})
        else:
            # For multiple assets
